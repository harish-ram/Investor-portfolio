...
public class StockAPIService {
    ...
    private static final String FALLBACK_API_URL = "https://api.example.com/stocks/";
    ...
    private StockPriceDTO fetchFromPrimaryAPI(String symbol) {
        try {
            ...
            LOGGER.warn("Primary API failed for " + symbol + ", attempting fallback", e);
            return null;
        }
    }
    ...
    private StockPriceDTO parseStockResponse(String jsonResponse, String symbol) {
        try {
            ...
            if (json.has("totalTradedVolume")) {
                String volumeStr = json.get("totalTradedVolume").getAsString();
                try {
                    dto.setVolume(Long.parseLong(volumeStr.replaceAll(",", "")));
                } catch (NumberFormatException e) {
                    LOGGER.debug("Could not parse volume: " + volumeStr);
                }
            }
